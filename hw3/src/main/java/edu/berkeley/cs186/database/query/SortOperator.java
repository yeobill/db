...
public class SortOperator  {
  ...
  public Run sortRun(Run run) throws DatabaseException {
    ...
    while(iter.hasNext()) records.add(iter.next());
  ...
  public Run mergeSortedRuns(List<Run> runs) throws DatabaseException {
    ...
    for(int i = 0; i < iterList.size(); i++){
      q.add(new Pair<>((Record) iterList.get(i).next(), i));
    }

    while(!q.isEmpty()){
      Pair tempPair = q.remove();
      Record tempRecord = (Record) (tempPair.getFirst());
      Integer i = (Integer) (tempPair.getSecond());
      recordList.add(tempRecord);
      if(iterList.get(i).hasNext()){
        pair = new Pair<>((Record) iterList.get(i).next(), i);
        q.add(pair);
      }
    }
    result.addRecords(recordList);
    return result;
  }
  ...
  public List<Run> mergePass(List<Run> runs) throws DatabaseException {
    int mergeBuffer = numBuffers - 1;
    int numRuns = runs.size() / mergeBuffer;
    int mark = 0;
    List<Run> result = new ArrayList<>();

    for(int i=0; i<numRuns; i++){
      int start = mergeBuffer*i;
      int end = mergeBuffer*(i+1);
      result.add(mergeSortedRuns(runs.subList(start, end)));
      mark = end;
    }
    if(mark < runs.size()){
      result.add(mergeSortedRuns(runs.subList(mark, runs.size())));
    }
    return result;
  }
  ...
  public String sort() throws DatabaseException {
    BacktrackingIterator<Record> iter = this.transaction.getRecordIterator(this.tableName);
    Run[] runs = new Run[numBuffers];
    ...
    int count = 0;
    while(iter.hasNext()){
      runs[count%numBuffers].addRecord((iter.next()).getValues());
      count++;
    }

    for (Run run : runs) {
      result.add(sortRun(run));
    }
