...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute){
    ...
    Iterator<RecordId> iter =  table.ridIterator();
    Record currRecord;
    float currValue;
    if (iter.hasNext()) {
      RecordId currRid = iter.next();
      try {
        currRecord = table.getRecord(currRid);
      } catch (DatabaseException d) {
        return;
      }
      this.minValue = this.quantization(currRecord, attribute);
    ...
    while (iter.hasNext()) {
      ...
      currValue = this.quantization(currRecord, attribute);
    ...
    float currMin = this.minValue;
    for (int i=0; i < this.numBuckets; i++) {
      this.buckets[i] = new Bucket<Float>(currMin, currMin+this.width);
      currMin += this.width;
    }
      
      //4. populate the data using the increment(value) method
      Iterator<RecordId> newIter = table.ridIterator();
    int i = this.numBuckets/2;
    int tempLow = 0;
    int tempHigh = this.numBuckets;

    while (newIter.hasNext()) {
      RecordId currRid = newIter.next();
      try {
        currRecord = table.getRecord(currRid);
      } catch (DatabaseException d) {
        return;
      ...
      currValue = this.quantization(currRecord, attribute);
      while (true) {
        if (this.width == 0 || currValue == this.maxValue) {
          this.buckets[this.numBuckets-1].increment(currValue);
          tempLow = 0;
          tempHigh = this.numBuckets;
          break;
        }

        if (currValue >= this.buckets[i].getStart() && currValue < this.buckets[i].getEnd()) {
          this.buckets[i].increment(currValue);
          tempLow = 0;
          tempHigh = this.numBuckets;
          break;
        } else {
          if (currValue < this.buckets[i].getEnd()) {
            tempHigh = i-1;
            i = ((tempLow + tempHigh) / 2);
          } else {
            tempLow = i+1;
            i = ((tempLow + tempHigh)/2);
          }
  ...
  private float [] allEquality(float qvalue){
    ...
    for (int i=0; i < this.numBuckets; i++) {
      if (qvalue == this.maxValue) {
        result[this.numBuckets-1] = (float) 1/this.buckets[i].getDistinctCount();
      } else {
        if (qvalue >= this.buckets[i].getStart() && qvalue < this.buckets[i].getEnd()) {
          result[i] = (float) 1/this.buckets[i].getDistinctCount();
        } else {
          result[i] = 0;
        }
      }
 ...
 /**
  ...
  private float [] allNotEquality(float qvalue){
    ...
    for (int i=0; i < this.numBuckets; i++) {
      if (qvalue == this.maxValue) {
        result[this.numBuckets-1] = 1.0f - ((float) 1/this.buckets[i].getDistinctCount());
      } else {
        if (qvalue >= this.buckets[i].getStart() && qvalue < this.buckets[i].getEnd()) {
          result[i] = 1.0f - ((float) 1/this.buckets[i].getDistinctCount());
        } else {
          result[i] = 1;
        }
      }
  ...
  private float [] allGreaterThan(float qvalue){
    ...
    for (int i=0; i < this.numBuckets; i++) {
      if (qvalue == this.maxValue) {
        result[this.numBuckets-1] = (( this.buckets[i].getEnd() - qvalue)/ this.width);
      } else {
        if (qvalue >= this.buckets[i].getStart() && qvalue < this.buckets[i].getEnd()) {
 ...
 /**
  ...
  private float [] allLessThan(float qvalue){
    ...
    for (int i=0; i < this.numBuckets; i++) {
      if (qvalue == this.maxValue) {
        result[this.numBuckets-1] = ((qvalue-this.buckets[i].getStart())/ this.width);
      } else {
        if (qvalue >= this.buckets[i].getStart() && qvalue < this.buckets[i].getEnd()) {
