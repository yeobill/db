...
//    }
    ...
    public BPlusTree(String filename) {
        PageAllocator pa = new PageAllocator(filename, false);
        Page page = pa.fetchPage(0);
        ...
        this.metadata = new BPlusTreeMetadata(pa, keySchema, order);
        this.root = BPlusNode.fromBytes(this.metadata, pageNum);
    ...
    public Iterator<RecordId> scanAll() {
      // TODO(hw2): Return a BPlusTreeIterator.
        return new BPlusTreeIterator(0, root.getLeftmostLeaf().getRids().size(), root.getLeftmostLeaf().getRightSibling());
    ...
    public void put(DataBox key, RecordId rid) throws BPlusTreeException {
      ...
      try {
          ...
          Optional<Pair<DataBox, Integer>> temp1 = this.root.put(key, rid);
          if (temp1.isPresent()) {
              Pair<DataBox, Integer> temp =  temp1.get();
              DataBox splitKey = temp.getFirst();
              Integer pageNum = temp.getSecond();
              keys.add(splitKey);
              children.add(this.root.getPage().getPageNum());
              children.add(pageNum);
              InnerNode newRoot = new InnerNode(this.metadata, keys,children);
              this.root = newRoot;
              writeHeader(this.headerPage.getByteBuffer());
          }
      }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
      // TODO(hw2): Add whatever fields and constructors you want here
        ...
        BPlusTreeIterator(int start, int end, Optional<LeafNode> rightSibling) {
            this.leaf = root.getLeftmostLeaf();
            this.start = start;
            this.rightSibling = rightSibling;
            this.rids = leaf.getRids();
            this.end = end;
        }
        BPlusTreeIterator(DataBox key) {
            this.leaf = root.get(key);
            RecordId startID = leaf.getKey(key).get();
            this.rids = leaf.getRids();
            this.start = rids.indexOf(startID);
            this.end = rids.size();
            this.rightSibling = leaf.getRightSibling();
      ...
      public boolean hasNext() {
        if (this.start >= end) {
            if (rightSibling.equals(Optional.empty())) {
                return false;
            }
        }
        return true;
      ...
      public RecordId next() {
        if(! this.hasNext()) {
          ...
          if (start >= end) {
              LeafNode nextLeft = leaf.getRightSibling().get();
              rids = nextLeft.getRids();
              leaf = nextLeft;
              start = 0;
              end = rids.size();
              rightSibling = leaf.getRightSibling();

          }
          result = rids.get(start);
          start++;
