...
class LeafNode extends BPlusNode {
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
    ...
    if(is_split) {
      List<DataBox> nextKeys = this.keys.subList(d, 2*d+1);
      List<RecordId> nextrids = this.rids.subList(d, 2*d+1);
      LeafNode leaf = new LeafNode(this.metadata, nextKeys, nextrids, this.rightSibling);
      keys = this.keys.subList(0, d);
      rids = this.rids.subList(0,d);
      Integer pageNum = leaf.getPage().getPageNum();
      DataBox newkey = leaf.keys.get(0);
  ...
  public static LeafNode fromBytes(BPlusTreeMetadata metadata, int pageNum) {
    ...
    int n = buf.getInt();
//    int size = buf.getInt();
    Optional rightsibling;
    if (next == -1) {
      rightsibling = Optional.empty();
    } else {
      rightsibling = Optional.of(next);
    }
